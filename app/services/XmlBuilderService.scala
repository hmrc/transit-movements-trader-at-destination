...
class XmlBuilderService {
  ...
  private def createXml(arrivalNotificationRequest: ArrivalNotificationRequest)(implicit dateTime: LocalDateTime) = {
    ...
    val childNodes: NodeSeq = {
      buildMetaNode(arrivalNotificationRequest.meta, arrivalNotificationRequest.messageCode.code) ++ buildHeaderNode(
        arrivalNotificationRequest.header,
        Format.dateFormatted(dateTime)) ++ buildTraderDestinationNode(arrivalNotificationRequest.traderDestination) ++ buildOfficeOfPresentationNode(
        arrivalNotificationRequest.customsOfficeOfPresentation) ++ buildEnRouteEventsNode(arrivalNotificationRequest)
    }
  ...
  private def buildMetaNode(meta: Meta, messageCode: String)(implicit dateTime: LocalDateTime): NodeSeq =
    <SynIdeMES1>
    {meta.syntaxIdentifier}
  </SynIdeMES1> <SynVerNumMES2>
    {meta.syntaxVersionNumber}
  </SynVerNumMES2> <MesSenMES3>
    {meta.messageSender.toString}
  </MesSenMES3> ++ buildOptionalElem(meta.senderIdentificationCodeQualifier, "SenIdeCodQuaMES4") ++ buildOptionalElem(meta.recipientIdentificationCodeQualifier,
                                                                                                                      "RecIdeCodQuaMES7") ++ <MesRecMES6>
    {meta.messageRecipient}
  </MesRecMES6> ++ <DatOfPreMES9>
    {Format.dateFormatted(dateTime)}
  </DatOfPreMES9> ++ <TimOfPreMES10>
    {Format.timeFormatted(dateTime)}
  </TimOfPreMES10> ++ <IntConRefMES11>
    {meta.interchangeControlReference.toString}
  </IntConRefMES11> ++ buildOptionalElem(meta.recipientsReferencePassword, "RecRefMES12") ++ buildOptionalElem(meta.recipientsReferencePasswordQualifier,
                                                                                                               "RecRefQuaMES13") ++ <AppRefMES14>
    {meta.applicationReference}
  </AppRefMES14> ++ buildOptionalElem(meta.priority, "PriMES15") ++ buildOptionalElem(meta.acknowledgementRequest, "AckReqMES16") ++ buildOptionalElem(
      meta.communicationsAgreementId,
      "ComAgrIdMES17") ++ <TesIndMES18>
    {meta.testIndicator}
  </TesIndMES18> ++ <MesIdeMES19>
    {meta.messageIndication}
  </MesIdeMES19> ++ <MesTypMES20>
    {messageCode}
  </MesTypMES20> ++ buildOptionalElem(meta.commonAccessReference, "ComAccRefMES21") ++ buildOptionalElem(meta.messageSequenceNumber, "MesSeqNumMES22") ++ buildOptionalElem(
      meta.firstAndLastTransfer,
      "FirAndLasTraMES23")

  private def buildHeaderNode(header: Header, arrivalNotificationDate: String): NodeSeq = <HEAHEA>
    <DocNumHEA5>
      {header.movementReferenceNumber}
    </DocNumHEA5>{buildOptionalElem(header.customsSubPlace, "CusSubPlaHEA66")}<ArrNotPlaHEA60>
      {header.arrivalNotificationPlace}
    </ArrNotPlaHEA60> <ArrNotPlaHEA60LNG>
      {header.languageCode}
    </ArrNotPlaHEA60LNG>{buildOptionalElem(header.arrivalAgreedLocationOfGoods, "ArrAgrLocCodHEA62") ++ buildOptionalElem(header.arrivalAgreedLocationOfGoods, "ArrAgrLocOfGooHEA63")}<ArrAgrLocOfGooHEA63LNG>
      {header.languageCode}
    </ArrAgrLocOfGooHEA63LNG>{buildOptionalElem(header.arrivalAgreedLocationOfGoods, "ArrAutLocOfGooHEA65")}<SimProFlaHEA132>
      {header.simplifiedProcedureFlag}
    </SimProFlaHEA132> <ArrNotDatHEA141>
      {arrivalNotificationDate}
    </ArrNotDatHEA141>
  </HEAHEA>

  private def buildTraderDestinationNode(traderDestination: TraderDestination): NodeSeq = <TRADESTRD>
    {buildOptionalElem(traderDestination.name, "NamTRD7") ++ buildOptionalElem(traderDestination.streetAndNumber, "StrAndNumTRD22") ++ buildOptionalElem(traderDestination.postCode, "PosCodTRD23") ++ buildOptionalElem(traderDestination.city, "CitTRD24") ++ buildOptionalElem(traderDestination.countryCode, "CouTRD25")}<NADLNGRD>
      {traderDestination.languageCode}
    </NADLNGRD>{buildOptionalElem(traderDestination.eori, "TINTRD59")}
  </TRADESTRD>

  private def buildOfficeOfPresentationNode(customsOfficeOfPresentation: CustomsOfficeOfPresentation): NodeSeq = <CUSOFFPREOFFRES>
    <RefNumRES1>
      {customsOfficeOfPresentation.presentationOffice}
    </RefNumRES1>
  </CUSOFFPREOFFRES>
  ...
  private def buildEnRouteEventsNode(arrivalNotificationRequest: ArrivalNotificationRequest): NodeSeq = arrivalNotificationRequest.enRouteEvents match {
    ...
    case Some(enRouteEvent) =>
      ...
      </CTLCTL>{buildIncident(event.eventDetails)(arrivalNotificationRequest)}
      </ENROUEVETEV>
      }
  }
