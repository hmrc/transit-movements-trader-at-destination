...
class ValidateInboundMessageActionSpec
    extends AnyFreeSpec
    with Matchers
    with MockitoSugar
  ...
  val actionRefiner: ValidateInboundMessageAction                = new ValidateInboundMessageAction()
  ...
  "refine" - {
    Seq(
      ...
      case (response, status) =>
        s"return an Inbound message if an Inbound message ($response) is found" in {
          actionRefiner
            .refine(MessageTransformRequest(Message(response, status), fakeArrivalRequest))
            .value mustBe Some(Success(Right(InboundMessageRequest(InboundMessage(response, status), fakeArrivalRequest))))
        }
    }
    Seq(
      ...
      case (response, st) =>
        s"return bad request error if an outbound response ($response) is found" in {
