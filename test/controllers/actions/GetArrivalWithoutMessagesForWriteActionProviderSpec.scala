...
class GetArrivalWithoutMessagesForWriteActionProviderSpec
    extends AnyFreeSpec
    with Matchers
    with MockitoSugar
  ...
  class Harness(getAndLock: GetArrivalWithoutMessagesForWriteActionProvider) {
  ...
  "get arrival for edit" - {
    ...
    "must lock an arrival, retrieve it, then unlock it when the arrival exists" in {
      ...
      running(application) {
        val actionProvider = application.injector.instanceOf[GetArrivalWithoutMessagesForWriteActionProvider]
