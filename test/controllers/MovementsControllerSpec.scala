  ...
  "MovementsController" - {
    ...
    "updateArrival" - {
      ...
      "must return InternalServerError if the database fails to amend the Arrival Movement" in {
        ...
        when(mockLockRepository.unlock(any())).thenReturn(Future.successful(()))
        when(mockArrivalMovementRepository.get(any())).thenReturn(Future.successful(Some(arrival)))
        when(mockArrivalMovementRepository.addNewMessage(eqTo(arrival.arrivalId), any())).thenReturn(Future.failed(new Exception))
      ...
      "must return InternalServerError and update the status to SubmissionFailed if sending the message upstream fails" in {
        ...
        running(application) {
          ...
          status(result) mustEqual INTERNAL_SERVER_ERROR
          verify(mockArrivalMovementRepository, times(1)).addNewMessage(eqTo(arrival.arrivalId), any())
        }
      }
    ...
    "getMessage" - {
      "must return Ok with the retrieved message" in {

        val message = Arbitrary.arbitrary[MovementMessageWithState].sample.value.copy(state = SubmissionSucceeded)
        val arrival = Arbitrary.arbitrary[Arrival].sample.value.copy(messages = Seq(message))
      ...
      "must return NOT FOUND" - {
        "when arrival is not found" in {
          val arrival = Arbitrary.arbitrary[Arrival].sample.value.copy(arrivalId = ArrivalId(1))
          ...
          running(application) {
            val request = FakeRequest(GET, routes.MovementsController.getMessage(ArrivalId(2), 0).url)
            val result  = route(application, request).value
